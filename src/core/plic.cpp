...
MemoryResult PLIC::read32(addr_t addr) const
{
    ...
    if (addr >= PRIORITY_BASE && addr < PRIORITY_BASE + PLICConfig::MAX_SOURCES * 4) {
    ...
    if (addr == CLAIM_ADDR) {
        ...
        for (u32 i = 1; i < PLICConfig::MAX_SOURCES; ++i) {
            if ((pending_bits_ & (1u << i)) && (enable_bits_ & (1u << i))) {
                if (priorities_[i] > threshold_ && priorities_[i] > best_prio) {
                    best = i;
        ...
        if (best != 0) {
            const_cast<PLIC*>(this)->pending_bits_ &= ~(1u << best);
            claimed_ = best;
        }
        return {best, 1, true};
    }
...
bool PLIC::valid_address(addr_t addr, size_t) const
{
...
MemoryResult PLIC::write32(addr_t addr, u32 value)
{
    ...
    if (addr == CLAIM_ADDR) {
        if (value > 0 && value < PLICConfig::MAX_SOURCES && value == claimed_) {
            claimed_ = 0;
            notify();
        }
        return {value, 1, true};
...
{
    if (source == 0 || source >= PLICConfig::MAX_SOURCES) return;
...
{
    for (u32 i = 1; i < PLICConfig::MAX_SOURCES; ++i) {
        if ((pending_bits_ & (1u << i)) && (enable_bits_ & (1u << i)) && priorities_[i] > threshold_)
            return true;
    }
    return false;
}
...
{
    if (source > 0 && source < PLICConfig::MAX_SOURCES && source == claimed_)
        claimed_ = 0;
}

void PLIC::set_priority(u32 source, u32 prio)
{
    if (source > 0 && source < PLICConfig::MAX_SOURCES) {
