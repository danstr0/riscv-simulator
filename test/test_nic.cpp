 ...
 *
 * Sections:
 *   1 (line 144) : MMIO registers - control, status, read/write, read-clear
 *   2 (line 205) : RX path - inject packet, DMA to memory, descriptor update
 *   3 (line 266) : TX path - write descriptor, DMA from memory, poll_tx
 *   4 (line 311) : Interrupts - mask, raise, clear, callback
 *   5 (line 385) : Interrupt coalescing - packet count, timer threshold
 *   6 (line 459) : Loopback - TX->RX round-trip
 *   7 (line 499) : Statistics - counters, latency tracking
 *   8 (line 540) : Edge cases - ring full, disabled, reset
 */
...
/**
 * Set up an NIC with a system memory region and a simple RX descriptor ring.
 * Returns {nic, sys_mem}.
 */
struct NicTestSetup {
    ...
    /**
     * Set up a simple RX ring at base_addr with count descriptors.
     *
    ...
    /** Set up a simple TX ring at base_addr with count descriptors. */
