...
TEST(mesi_read_miss_exclusive) {
    ...
    s.main_mem->write32(0x100, 0xDEADBEEF);

    // Core 0 reads — no one else has it
    s.ctrl.handle_read_miss(0, 0x100);

    // After handling, controller should mark core 0 as Exclusive
    ASSERT_EQ(s.ctrl.line_state(0, 0x100), MESIState::Exclusive);
    ASSERT_EQ(s.ctrl.line_state(1, 0x100), MESIState::Invalid);
    return true;
}
...
TEST(mesi_read_shared_from_exclusive) {
    ...
    s.ctrl.handle_read_miss(1, 0x100);
    ASSERT_EQ(s.ctrl.line_state(0, 0x100), MESIState::Shared);
    ASSERT_EQ(s.ctrl.line_state(1, 0x100), MESIState::Shared);
    return true;
}
...
TEST(mesi_read_from_modified) {
    ...
    s.l1_0->write32(0x100, 0xCAFEBABE);
    s.ctrl.handle_write_miss(0, 0x100);  // E->M
    ASSERT_EQ(s.ctrl.line_state(0, 0x100), MESIState::Modified);
    ...
    ASSERT_EQ(s.main_mem->read32(0x100).value, 0xCAFEBABEu);
...
TEST(mesi_write_miss_modified) {
    ...
    s.ctrl.handle_write_miss(0, 0x200);
    ASSERT_EQ(s.ctrl.line_state(0, 0x200), MESIState::Modified);
    ASSERT_EQ(s.ctrl.line_state(1, 0x200), MESIState::Invalid);
    return true;
}
...
TEST(mesi_exclusive_to_modified) {
    ...
    // Core 0 reads -> Exclusive
    s.ctrl.handle_read_miss(0, 0x300);
    ASSERT_EQ(s.ctrl.line_state(0, 0x300), MESIState::Exclusive);

    // Core 0 writes -> Modified (silent, no invalidations needed)
    u32 lat = s.ctrl.handle_write_miss(0, 0x300);
    ASSERT_EQ(s.ctrl.line_state(0, 0x300), MESIState::Modified);
...
TEST(mesi_shared_to_modified_invalidates) {
    ...
    s.ctrl.handle_read_miss(0, 0x400);
    s.ctrl.handle_read_miss(1, 0x400);
    ASSERT_EQ(s.ctrl.line_state(0, 0x400), MESIState::Shared);
    ASSERT_EQ(s.ctrl.line_state(1, 0x400), MESIState::Shared);
    ...
    s.ctrl.handle_write_miss(0, 0x400);
    ASSERT_EQ(s.ctrl.line_state(0, 0x400), MESIState::Modified);
    ASSERT_EQ(s.ctrl.line_state(1, 0x400), MESIState::Invalid);
...
TEST(mesi_write_steals_modified) {
    auto s = CoherenceSetup::create();
    s.main_mem->write32(0x500, 100);

    // Core 0 gets line, writes -> Modified
    s.ctrl.handle_read_miss(0, 0x500);
    s.l1_0->read32(0x500);
    s.l1_0->write32(0x500, 999);
    s.ctrl.handle_write_miss(0, 0x500);
    ASSERT_EQ(s.ctrl.line_state(0, 0x500), MESIState::Modified);

    // Core 1 writes to same line -> core 0 writes back and is invalidated
    s.ctrl.handle_write_miss(1, 0x500);
    ASSERT_EQ(s.ctrl.line_state(0, 0x500), MESIState::Invalid);
    ASSERT_EQ(s.ctrl.line_state(1, 0x500), MESIState::Modified);

    // Core 0's dirty data should have been written back to memory
    ASSERT_EQ(s.main_mem->read32(0x500).value, 999u);
    return true;
}
...
TEST(mesi_eviction_cleans_directory) {
    ...
    s.ctrl.handle_read_miss(0, 0x600);
    ASSERT_EQ(s.ctrl.num_tracked_lines(), 1u);

    s.ctrl.notify_eviction(0, 0x600);
    ASSERT_EQ(s.ctrl.line_state(0, 0x600), MESIState::Invalid);
...
TEST(mesi_eviction_partial) {
    ...
    s.ctrl.handle_read_miss(0, 0x700);
    s.ctrl.handle_read_miss(1, 0x700);
    ASSERT_EQ(s.ctrl.num_tracked_lines(), 1u);

    // Core 0 evicts — core 1 still has it
    s.ctrl.notify_eviction(0, 0x700);
    ASSERT_EQ(s.ctrl.line_state(0, 0x700), MESIState::Invalid);
    ASSERT_EQ(s.ctrl.line_state(1, 0x700), MESIState::Shared);
...
TEST(mesi_data_visible_after_coherence) {
    auto s = CoherenceSetup::create();
    s.main_mem->write32(0x900, 0);

    // Core 0 reads and writes
    s.ctrl.handle_read_miss(0, 0x900);
    s.l1_0->read32(0x900);
    s.l1_0->write32(0x900, 42);
    s.ctrl.handle_write_miss(0, 0x900);

    // Core 1 wants to read — triggers coherence
    s.ctrl.handle_read_miss(1, 0x900);

    // The writeback should have pushed 42 to main memory
    // Core 1's L1 will fetch from main memory
    ASSERT_EQ(s.main_mem->read32(0x900).value, 42u);
    return true;
}
